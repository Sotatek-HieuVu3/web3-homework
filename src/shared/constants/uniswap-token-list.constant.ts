import { IUniswapToken } from 'src/shared/interfaces/uniswap-token.interface';

export const UNISWAP_TOKEN_MAP: Map<number, IUniswapToken[]> = new Map();

const UNISWAP_TOKEN_LIST = [
  {
    chainId: 1,
...
UNISWAP_TOKEN_LIST.forEach((token) => {
  if (!UNISWAP_TOKEN_MAP.has(token.chainId)) {
    UNISWAP_TOKEN_MAP.set(token.chainId, []);
  }
  UNISWAP_TOKEN_MAP.get(token.chainId).push(token);
});
